...
select user_id, count(order_date), sum(price) from
(select c.*, d.price from
(select s.*, g.gold_signup_date
 from sales s inner join goldusers_signup g
	 on s.user_id = g.user_id
 where order_date <= gold_signup_date) c inner join product d on c.product_id = d.product_id)e
group by user_id;

-- Q.9 If buying each product generates points for eg. 5 Rs. = 2 zomato points and each product has different purchasing points.
--     eg. For p1 r Rs. = 1 zomato point, for p2 10 Rs. = 5 zomato points and for p3 5 Rs. = 1 zomato point, 2 Rs. = 1 zomato point.
...
--     earn 5 zomato points for every 10rs spent who earned more more 1 or 3 and what was their point earning in first yr?
...
(SELECT c.*, 
        cast((case when gold_signup_date is null then 0 else rank() over(partition by user_id order by order_date desc) end) as varchar) as rnk from 
(select a.user_id, a.order_date, a.product_id, b.gold_signup_date 
